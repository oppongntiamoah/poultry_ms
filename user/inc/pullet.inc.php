...
//inserting data into medicine
    ...
    //EMAIL EXIST
    $result = $mysqli->query("SELECT * FROM total_pullet WHERE batch_id = '$batch' ") or die($mysqli->error());
    if ($result->num_rows > 0) {
        $_POST['msg'] = "<script>alert('Batch ID already exists!');</script>";
        header("location: ../pullet.php");
    }
        ...
        if (!empty($batch)) {
            if (!empty($qty)) {
                if (!empty($updateby)) {
                    $sql = "INSERT INTO total_pullet(batch_id, quantity, updated_by) 
